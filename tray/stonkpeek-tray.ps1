...
$script:WidgetPaint      = @{
  Name = "StonkPeek"; Price = $null; Day = ""; Sub = ""
...
function Set-WidgetContent {
  ...
  if ($script:Boss) { Set-WidgetPlaceholder "StonkPeek" "사장님 모드"; return }
  if ($script:WidgetStale -or $script:WidgetFrames.Count -eq 0) {
    Set-WidgetPlaceholder "StonkPeek" "데몬 꺼짐 — stonkpeek start"; return
  }
...
# config.ts의 탐색 순서(cwd → 프로젝트 루트 → ~/.stonkpeek) 중 트레이 입장에서
# 결정적인 두 곳만 그대로 따라간다 — cwd는 로그인 자동실행 땐 System32라 의미 없음.
function Get-ProjectRoot { Split-Path -Parent $PSScriptRoot }

function Get-ConfigPath {
  $projectConfig = Join-Path (Get-ProjectRoot) "stonkpeek.config.json"
  if (Test-Path $projectConfig) { return $projectConfig }
  return (Join-Path $env:USERPROFILE ".stonkpeek\config.json")
}
...
function Test-AutostartInstalled {
  Test-Path (Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\Startup\stonkpeek-autostart.vbs")
}
